...
mkdir app/frontend/images
mkdir app/frontend/stylesheets
vim app/frontend/packs/application.js
```
...
```sh
vim app/frontend/stylesheets/bootstrap.scss
```
...
```sh
vim app/frontend/images/logo.svg
```
...
```sh
git add . && git commit -m "feat: use bootstrap with webpack"
yarn changelog:check
yarn changelog
git add . && git commit -m "docs: update changelog to 0.6.0"
git tag 0.6.0 HEAD -m "bootstrap with webpack"
git push --all && git push --tags
```
